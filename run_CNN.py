...
import numpy as np
import random
from model import QNetwork_CNN, get_network_input_for_CNN
...
gridsize = 15
model = QNetwork_CNN(gridsize+2, gridsize+2, output_dim=5)
model_target = QNetwork_CNN(gridsize+2, gridsize+2, output_dim=5)
...
def run_episode(num_games):
    ...
    while run:
        state = get_network_input_for_CNN(board.snake, board.apple)
        ...
        next_state = get_network_input_for_CNN(board.snake, board.apple)
...
def learn(num_updates, batch_size):
    ...
    for i in range(num_updates):
        ...
        Q_targets_next = torch.max(next_q_value.detach(), 1)[0] * (torch.ones(dones.size()) - dones)
