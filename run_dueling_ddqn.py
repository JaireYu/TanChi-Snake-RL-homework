...
import numpy as np
import random
from model import QNetwork, get_network_input, Dueling_QNetwork
...
setup_seed(1234)
writer = SummaryWriter(log_dir='log_Dueling_ddqn')
cpk_epochs = 22000
ckpt_path = './dir_chk_len/Snake_Dueling_ddqn_{}'.format(cpk_epochs)
cpk = torch.load(ckpt_path)
model = Dueling_QNetwork(input_dim=10, hidden_dim=20, output_dim=5)
model_target = Dueling_QNetwork(input_dim=10, hidden_dim=20, output_dim=5)

model.load_state_dict(cpk)
model_target.load_state_dict(cpk)
...
def train(init_epoch=0):
    ...
    for i_episode in range(init_epoch,num_episodes + 1):
        ...
        if i_episode % 250 == 0 and i_episode > 0:
            torch.save(model.state_dict(), './dir_chk_len/Snake_Dueling_ddqn_{}'.format(i_episode))

    return scores_array, avg_scores_array, avg_len_array, avg_max_len_array

if __name__ == "__main__":
    scores, avg_scores, avg_len_of_snake, max_len_of_snake = train(cpk_epochs)
